...
class Train:
    def __init__(self, param_, data_, test_, data_process,eav):
    ...
    def Train_RS(self):
        ...
        for batchID in range(num_batches):
            ...
            negatives_np_multi = self.generate_negative_samples(self.train_matrix, self.args.neg_samples,
                                                                self.args.sets_of_neg_samples)
            ...
            self.epoch_loss =self.epoch_loss+ self.loss_rs.item()
            self.optimizer_rs.zero_grad()
            self.loss_rs.backward()
            self.epoch_loss =self.epoch_loss/ num_batches
    ...
    def Train_KGE(self, kg):
        ...
        for batchID in range(num_batches):
            ...
            batch_head = torch.from_numpy(batch_head).type(torch.LongTensor).to(self.device)
    ...
    def evaluation(self, model_, test_, topk=20, save=False):
        ...
        precision, recall, MAP, ndcg, DIF = [], [], [], [],[]
        # print("test_Y_set:",test_Y_set)
        ...
        for k in [5, 10, 15, 20]:
            ...
            DIF.append(DIF_k(test_Y_set, pred_list, k,self.eav))
        return precision, recall, MAP, ndcg,DIF
    ...
    def run(self, show_loss=True):
        ...
        for epoch in range(self.args.n_epochs):
            ...
            if (epoch + 1) % 5 == 0:
                ...
                precision, recall, MAP, ndcg, DIF = self.evaluation(self.test_model, self.test_, topk=20, save=False)
            ...
            if (epoch + 1) == self.args.n_epochs:
                self.test_model.eval()
                precision, recall, MAP, ndcg, DIF = self.evaluation(self.test_model, self.test_, topk=20, save=True)
                print('precision : ', precision)
                print('recall : ', recall)
                print('MAP : ', MAP)
                print('NDCG : ', ndcg)
